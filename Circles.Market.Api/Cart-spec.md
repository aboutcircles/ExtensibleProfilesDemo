...
3. **exchanges** a valid basket for an immutable **order** id.
...
```json
{
  ...
  "ageProof": { "@type": "Person" },                    // minimal shape, more below
...
```json
{
  ...
  "offerSnapshot": {
    ...
    "checkoutPageURLTemplate": "https://...{orderId}"   // optional, aligns with schema.org
  }
}
```
...
-> `200 ValidationResult` (does not mutate)
...
* Payments can reference `Order.orderNumber`/`orderId` and/or `paymentUrl`. (Out of scope to implement here.)
...
* **Observability**: emit `BasketCreated`, `BasketPatched`, `BasketValidated`, `BasketCheckedOut`, `OrderCreated`, `BasketExpired` (with `basketId`, `orderId`, `buyer`, `operator`).
...
* Keep your existing `circles:checkout`/`circles:getOrderId` terms; they can co-exist with schema.org's `checkoutPageURLTemplate` on `Offer`. ([Schema.org][5])
