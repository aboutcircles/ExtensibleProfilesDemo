...
internal static class SafeHelpers
{
    ...
    /// <summary>
    /// Directly calls the updateMetadataDigest method on the NameRegistry contract for a given address
    /// </summary>
    internal static async Task UpdateProfileDigestAsync(
        Web3 web3, string avatarAddress, Account signer, byte[] digest32, 
        CancellationToken ct = default)
    {
        if (web3 == null) throw new ArgumentNullException(nameof(web3));
        if (string.IsNullOrEmpty(avatarAddress)) throw new ArgumentNullException(nameof(avatarAddress));
        if (signer == null) throw new ArgumentNullException(nameof(signer));
        if (digest32 == null) throw new ArgumentNullException(nameof(digest32));
        
        // Check that the signer address matches the avatar address
        if (!signer.Address.Equals(avatarAddress, StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException($"Avatar {avatarAddress} must equal tx-signer {signer.Address}");
        }
        
        // Name Registry interface (minimal for updateMetadataDigest)
        var abi = @"[
            {
              ""inputs"": [
        ...
        var contract = web3.Eth.GetContract(abi, NameRegistryAddress);
        var updateFunction = contract.GetFunction("updateMetadataDigest");
        ...
        var receipt = await updateFunction.SendTransactionAndWaitForReceiptAsync(
            ...
            functionInput: new object[] { digest32 });
            
        if (receipt.Status.Value == 0)
            throw new InvalidOperationException("Transaction failed: " + receipt.TransactionHash);
    }
